...
<section class="ftco-section bg-light" id="blog-section" style="position:relative; margin-top:-96px; min-height: 800px;">
...
if($_GET['do']='sagnebi')
{
  $where = '';  
	...
	if($_GET['stil']=="ქიმია") 
$where.="sagnebi LIKE '|%ქიმია%|' AND"; 
	
	if($_GET['stil']=="ფიზიკა") 
$where.="sagnebi LIKE '|%ფიზიკა%|' AND";
	
	if($_GET['stil']=="ინგლისური ენა") 
$where.="sagnebi LIKE '|%ინგლისური ენა%|' AND";
	
	if($_GET['stil']=="რუსული ენა") 
$where.="sagnebi LIKE '|%რუსული ენა%|' AND";
	
	if($_GET['stil']=="ისტორია") 
$where.="sagnebi LIKE '|%ისტორია%|' AND";
	
	if($_GET['stil']=="გეოგრაფია") 
$where.="sagnebi LIKE '|%გეოგრაფია%|' AND";
	
	 if($_GET['stil']=="ბიოლოგია") 
$where.="sagnebi LIKE '|%ბიოლოგია%|' AND"; 
	
	if($_GET['stil']=="ბუნებისმეტყველება") 
$where.="sagnebi LIKE '|%ბუნებისმეტყველება%|' AND";  
	
	if($_GET['stil']=="სამოქალაქო განათლება") 
$where.="sagnebi LIKE '|%სამოქალაქო განათლება%|' AND"; 
	
	if($_GET['stil']=="ინფორმაციული და საკომუნიკაციო ტექნოლოგიები") 
$where.="sagnebi LIKE '|%ინფორმაციული და საკომუნიკაციო ტექნოლოგიები%|' AND";
	
	if($_GET['stil']=="მათემატიკა (I-VI)") 
$where.="sagnebi LIKE '|%მათემატიკა (I-VI)%|' AND";
	
	if($_GET['stil']=="მათემატიკა (VII-XII)") 
$where.="sagnebi LIKE '|%მათემატიკა (VII-XII)%|' AND";
	
	if($_GET['stil']=="ქართული (I-VI)") 
$where.="sagnebi LIKE '|%ქართული (I-VI)%|' AND";
	
	if($_GET['stil']=="ქართული (VII-XII)") 
$where.="sagnebi LIKE '|%ქართული (VII-XII)%|' AND";
	
	if($_GET['stil']=="სახვითი და გამოყენებითი ხელოვნება") 
$where.="sagnebi LIKE '|%სახვითი და გამოყენებითი ხელოვნება%|' AND";
	
	if($_GET['stil']=="გერმანული ენა") 
$where.="sagnebi LIKE '|%გერმანული ენა%|' AND";
	
   if($_GET['stil']=="ფრანგული ენა") 
$where.="sagnebi LIKE '|%ფრანგული ენა%|' AND";	
	
	if($_GET['stil']=="მუსიკა") 
$where.="sagnebi LIKE '|%მუსიკა%|' AND";
	
	if($_GET['stil']=="ფიზიკური აღზრდა და სპორტი") 
$where.="sagnebi LIKE '|%ფიზიკური აღზრდა და სპორტი%|' AND";
...
color:red;
 ...
 if($cnt['cnt']==0)
 {
		  ...
		  {
echo "<i class='fa fa-warning yellow-color'></i>".'&nbsp&nbspმოიძებნა&nbsp&nbsp'.(intval($cnt['cnt'])).' მასალა';  
		  }
