document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("click", function (event) {
        var isClickInside =
            uploadSidebar.contains(event.target) ||
    ...
    canvas.on("mouse:down", function (options) {
        if (
            options.target &&
            (options.target === topTextObject ||
                options.target === bottomTextObject)
        ) {
    ...
    document.getElementById("top_text").addEventListener("input", function () {
        console.info("Top text input changed");
        topTextObject.set({ text: this.value });
        canvas.renderAll();
    });

    document
        .getElementById("bottom_text")
    ...
    function adjustCurvedTextBoundingBox(textObj) {
        setTimeout(() => {
            ...
            let newLeft =
                boundingBox.left + boundingBox.width / 2 - textObj.width / 2;
            let newTop =
                boundingBox.top + boundingBox.height / 2 - textObj.height / 2;
    ...
    function applyCurvedTextEffect(textObj) {
        ...
        let path = new fabric.Path(
            `M 0,${radius} A ${radius},${radius / 1.5} 0 1,1 ${radius * 2
            },${radius}`,
            {
                fill: "",
                stroke: "",
        ...
        button.addEventListener("click", function () {
            ...
            switch (style) {
                ...
                case "shadow":
                    activeText.set(
                        "shadow",
                        activeText.shadow ? "" : "2px 2px 5px rgba(0,0,0,0.3)"
                    );
                    break;
                case "curved":
                    applyCurvedTextEffect(activeText);
    ...
    function applyTextStyle(style) {
        ...
        if (style === "bold") {
            ...
            activeText.set(
                "shadow",
                activeText.shadow ? null : "2px 2px 5px rgba(0,0,0,0.3)"
            );
     ...
     */
    let color_switcher_btns = document.querySelectorAll(".color-option");

    color_switcher_btns.forEach((btn) => {
        btn.addEventListener("click", function () {
            let frontImage = this.getAttribute("data-image");
            ...
            fabric.Image.fromURL(frontImage, function (img) {
                img.set({
                    left: 0,
                    top: 0,
                ...
                topTextObject = new fabric.Textbox(topText, {
                    left: canvas.width / 2,
                    top: 50,
                    fontSize: 30,
                    fill: "#000000",
                ...
                bottomTextObject = new fabric.Textbox(bottomText, {
                    left: canvas.width / 2,
                    top: 400,
                    fontSize: 30,
                    fill: "#000000",
