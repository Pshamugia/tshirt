...
function makeMenu($root_id, $space)
{
	global $conn;
	...
	while($menu=mysqli_fetch_assoc($res))
	{
		...
		if($cnt['cnt'])
		{
			echo '<option value="'.$menu['id'].'">'.str_repeat("&nbsp;&nbsp;&nbsp;&nbsp;", $space).$menu['title_ka'].'</a>';
			makeMenu($menu['id'], $space+1);
		}
		else
		{
			echo '<option value="'.$menu['id'].'">'.str_repeat("&nbsp;&nbsp;&nbsp;&nbsp;", $space).$menu['title_ka'].'</a>';		
...
// Escape and process data before inserting into the database to prevent SQL injection
// Check if $_REQUEST['kategory'] is set and is an array
if (isset($_REQUEST['kategory']) && is_array($_REQUEST['kategory'])) {
    // Escape each element in the array and implode with '|'
    $kategory = implode('|', array_map(function($item) use ($conn) {
        return mysqli_real_escape_string($conn, $item);
    }, $_REQUEST['kategory'])) . '|';
} else {
    // Handle the case where $_REQUEST['kategory'] is not set or is not an array
    $kategory = ''; // Set a default value or handle the error as per your requirement
}
$subkat = mysqli_real_escape_string($conn, $_REQUEST['subkat']);
$satauri_ka = mysqli_real_escape_string($conn, $_REQUEST['satauri_ka']);
$satauri_en = mysqli_real_escape_string($conn, $_REQUEST['satauri_en']);
$avtori = mysqli_real_escape_string($conn, $_REQUEST['avtori']);
$avtori2 = mysqli_real_escape_string($conn, $_REQUEST['avtori2']);
$agwera_ka = mysqli_real_escape_string($conn, $_REQUEST['agwera_ka']);
$agwera_en = mysqli_real_escape_string($conn, $_REQUEST['agwera_en']);
$full_ka = mysqli_real_escape_string($conn, $_REQUEST['full_ka']);
$full_en = mysqli_real_escape_string($conn, $_REQUEST['full_en']);
$date = date('Y-m-d H:i:s');
$news_date = strtotime($_REQUEST['news_date']);
$ena = mysqli_real_escape_string($conn, $_REQUEST['ena']);
$menu = mysqli_real_escape_string($conn, $_REQUEST['menu']);
$view_count = intval($_REQUEST['view_count']);
$eng_geo = mysqli_real_escape_string($conn, $_REQUEST['eng_geo']);
$img_description = mysqli_real_escape_string($conn, $_REQUEST['img_description']);
$hour = mysqli_real_escape_string($conn, $_REQUEST['hour']);
$tags_geo = mysqli_real_escape_string($conn, $_REQUEST['tags_geo']);
$tags_eng = mysqli_real_escape_string($conn, $_REQUEST['tags_eng']);
$hidden = intval($_REQUEST['hidden']);
$pos = intval($_REQUEST['pos']);

// Construct the SQL query
$sql = "INSERT INTO kultura_cxrili (kategory, subkat, upload, satauri_ka, satauri_en, avtori, avtori2, agwera_ka, agwera_en, full_ka, full_en, date, news_date, ena, menu, view_count, eng_geo, img_description, hour, tags_geo, tags_eng, hidden, pos) 
        VALUES ('$kategory', '$subkat', '$b[0]', '$satauri_ka', '$satauri_en', '$avtori', '$avtori2', '$agwera_ka', '$agwera_en', '$full_ka', '$full_en', '$date', '$news_date', '$ena', '$menu', '$view_count', '$eng_geo', '$img_description', '$hour', '$tags_geo', '$tags_eng', '$hidden', '$pos')";
...
<br>
 ...
 საათი და წუთი <br>
 <input type="text" name="hour" id="hour" value="<?php date_default_timezone_set('+0400'); ?>"> 
