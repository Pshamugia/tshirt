...
display:block; 
  ...
  <div style="background-color:#F0F0F0; padding:10px; margin-bottom:15px;"> <div id="indexs" style="color:#FFFFFF;">
   ...
   top:3px;"> +</span> <span style="position:relative; top:-3px;"> Add new Articles </span> </a> </div> </div>
...
<?php
	...
	*/
	$query = "SELECT COUNT(*) as num FROM $tbl_name where kategory!='images' and kategory!='ფოტო' and kategory!='ვიდეო' AND eng_geo='Eng'";
	...
	$sql = "SELECT * FROM $tbl_name where kategory!='images' and kategory!='ფოტო' and kategory!='ვიდეო' AND eng_geo='Eng' order by id desc LIMIT $start, $limit";
...
<input name="delete" type="submit"    value="Delete" style="font-size:12px;"> </div>

<div style="padding-top:3px; width:62px; display:table-cell;">
  <input name="ra" type="button" value="Edit" onclick="window.location.href='index.php?do=engupdate&id=<? echo $b['id']; ?>';"  style="font-size:12px; margin-right:10px; margin-left:3px;"> </div>
   
<div style="padding-top:3px; width:60px; display:table-cell;">
 <input name="view" type="button" formtarget="_new" value="View"  onclick="window.open('../index.php?do=full&id=<? echo $b['id']; ?>')" style="font-size:12px;">  </div>
