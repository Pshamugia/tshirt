...
$sql="insert into kultura_cxrili (kategory, subkat, upload, eng_geo, img_description, satauri, avtori, subcat_id, person_id, agwera, full, ena, menu, date, pos, news_date)values('$_REQUEST[kategory]', '$_REQUEST[subkat]', '$b', '$_POST[eng_geo]', '$_POST[img_description]', '$_POST[satauri]', '$_POST[avtori]', '$partyId', '$personId', '$_REQUEST[agwera]', '$_REQUEST[full]', '$_REQUEST[ena]', '$_REQUEST[menu]', '$date', '0', '$_REQUEST[news_date]')";
  ...
  <li style="background-color:#E8BA33; width:102px; height:25px; position:relative; margin-top:-6px; color:#FFFFFF;"> <span style="position:relative; font-size:26px; font-weight:bold; left:5px; top:-1px;"> &equiv; </span> <span style="position:relative; top:-6px; left:5px;"> Content </span>  </li>
    ...
    <li class="sousmenu"><a href="#">მენეჯერები</a>
      <ul class="niveau2">
        ...
        <li><a href="index.php?do=category">კატეგორიების დამატება</a> </li>
        <li><a href="index.php?do=subcategory">ქვეკატეგორიების დამატება</a> </li>
    ...
    <li><a href="index.php?do=authors">ავტორები</a> </li>
      ...
      <ul class="niveau2">
        ...
        <li><a href="index.php?do=enghome">English</a></li> 
             ...
             <li><a href="http://www.pencenter.ge/roundcube/" target="_blank">ელფოსტა</a></li>
...
$sql="insert into category (catname)values('$_REQUEST[catname]')";
mysql_query($sql);
...
<form action="" method="post" enctype="multipart/form-data" >
  <p>
    <? 
if (isset($_REQUEST['cat_delete']))
mysql_query("delete from category where id='$_REQUEST[subcat_id]' ");
mysql_query("delete from subcategory where subcat_id='$_REQUEST[subcat_id]' ");


?> 
    
    <br />დაამატე კატეგორია
  <input type="text" name="catname" />
  <input type="submit" name="adcat" style="border-radius: 5px; 
...
border: 1px solid #999; background-color:transparent;">
  ...
  წაშალე კატეგორია
    <select name="subcat_id">
      <?
	 $avt=mysql_query("select * from  category order by id desc");
	 while($avts_id=mysql_fetch_array($avt))
	 {
	echo "<option value='".$avts_id['id']."'>".$avts_id['catname']."</option>";			 
	 }
     ?>
    </select> 
    <input type="submit" name="cat_delete" value="delete" style="border-radius: 5px; 
-moz-border-radius: 5px; 
-webkit-border-radius: 5px; 
border: 1px solid #999; background-color:transparent;" />
  </p> 
